...
public class TrackVersionTest {
	...
	public void testInitialTrack() {
		...
		track.setKeyEnd("G");
		track.setKeyStart("E");
		track.setCreateUser("TrackVersionTest.testInitialTrack");
		Timestamp currentTimestamp = new Timestamp(new java.util.Date().getTime());
		track.setCreateTimestamp(currentTimestamp);
		track.setUpdateUser(track.getCreateUser());
		track.setUpdateTimestamp(track.getCreateTimestamp());
	...
	/**
	 * Test logic to get a single track This requires record id #1 to be present
	 */
	@Test(timeout = 10000)
	...
	/**
	 * Update a single ride This test will update the test record ID 1 It will do
	 * this by doing a "put" to the single track version url instead of a "get"
	 */
	@Test(timeout = 10000)
	...
	/**
	 * Utililty method for testing - Retrieves all trackversion objects from the
	 * database
	 * 
	...
	public List<TrackVersion> getTrackVersions() {
		...
		// Issue the rest call
		ResponseEntity<List<TrackVersion>> trackVersionsResponse = restTemplate.exchange(url, HttpMethod.GET, null,
	...
	public void testBatchUpdate() {
		...
		for (TrackVersion tv : tvList) {
			...
			/*
			 * Solution 2 java.util.Date today = new java.util.Date(); Timestamp ts = new
	...
	public void testDeleteTrack() {
		...
		// ** Looks like same size is returned from getTrackVersions even after the list
		// is updated **
	...
	/**
	 * The following test logic to attempt to delete track 1 resulting in a
	 * dataaccessexception
	 */
	@Test
